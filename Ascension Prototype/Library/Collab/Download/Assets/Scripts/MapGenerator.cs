...
public class MapGenerator : MonoBehaviour
{
    ...
    void CreateRoom(int[,] route,int row, int col, int number, int maxRooms)
    {
        ...
        if (DetermineRoom(route, row, col, maxRooms) != null)
            Instantiate(Resources.Load<GameObject>(DetermineRoom(route, row, col, maxRooms) + "/" + ((int)Random.Range(1, 5)).ToString()), new Vector3(x, y, 0), Quaternion.identity);
    ...
    void PlanRoute(int[,] route)
    {
        ...
        do
        {
            ...
            if (row == 0 && column == 0 && route[0,1] != 0)
                order = 15;
            if (row == 0 && column == route.GetLength(1) - 1 && route[0,1] != 0)
                order = 15;
        }
